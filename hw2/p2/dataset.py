...
def get_dataloader(dataset_dir, batch_size=1, split='test'):
    ...
    if split == 'train':
        transform = transforms.Compose([
            ...
            #transforms.Compose([transforms.GaussianBlur(7,3)]),
...
class CIFAR10Dataset(Dataset):
    def __init__(self, dataset_dir, split='test', transform=None):
        ...
        self.dataset_dir = dataset_dir #direction
    ...
    def __getitem__(self, index):
        ...
        pass
        img_transfrom = self.transform(img)
        if self.split != "test":
            return {'images': img_transfrom, 'labels': self.labels[index]}
        else:
            return {'images': img_transfrom}
